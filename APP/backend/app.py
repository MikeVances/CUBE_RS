...
Центральный API для мобильного приложения и веб-интерфейса
"""
...
def generate_signature(payload: str, timestamp: str) -> str:
    ...
    signature = hmac.new(
        api_config.api_secret.encode("utf-8"), message.encode("utf-8"), hashlib.sha256
    ).hexdigest()
    return signature
    ...
    try:
        ...
        if method.upper() == "GET":
            ...
            response = requests.post(
                url, headers=headers, json=data, timeout=api_config.timeout
            )
        else:
...
def api_status():
    ...
    if not api_config.is_configured():
        return (
            jsonify(
                {"status": "error", "message": "API не настроен", "configured": False}
            ),
            500,
        )
...
def get_current_data():
    ...
    if data:
        return jsonify(
            {"status": "success", "data": data, "timestamp": datetime.now().isoformat()}
        )
    else:
        return (
            jsonify({"status": "error", "message": "Не удалось получить данные"}),
            503,
        )
...
def get_statistics():
    ...
    if data:
        ...
        return (
            jsonify({"status": "error", "message": "Не удалось получить статистику"}),
            503,
        )
...
def tailscale_devices():
    ...
    service = get_tailscale_service()
    if not service:
        return jsonify({"status": "error", "message": "Tailscale не настроен"}), 503
...
def check_farm_connectivity():
    ...
    if not data or "tailscale_ip" not in data:
        return jsonify({"status": "error", "message": "Требуется tailscale_ip"}), 400
...
def register_device():
    ...
    if not data:
        return (
            jsonify({"status": "error", "message": "Отсутствуют данные запроса"}),
            400,
        )
        ...
        return jsonify(
            {
                ...
                "filters": {"type": device_type, "status": status},
                "timestamp": datetime.now().isoformat(),
            }
        )
...
def approve_registration_request(request_id):
    ...
    try:
        ...
        if success:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "Запрос не найден или уже обработан"}
                ),
                404,
            )
...
def revoke_device(device_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({"status": "error", "message": "Устройство не найдено"}), 404
...
if __name__ == "__main__":
    ...
    if not tailscale_config.is_configured():
        logger.warning(
            "⚠️ Tailscale не настроен! Для активации установите переменные окружения:"
        )
