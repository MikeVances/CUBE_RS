...
class TypedModbusGateway:
    ...
    def __init__(
        ...
        self.host = host
        self.port = port
        # Resolve DB path to apps/edge/storage/kub_data.db by default
        if db_path is None:
            try:
                # Try to use the same resolver as storage
                from .modbus_storage import DB_FILE as _DB

                self.db_path = Path(_DB)
            except Exception:
                self.db_path = (
                    Path(__file__).resolve().parents[1] / "storage" / "kub_data.db"
                )
        else:
    ...
    async def _register_default_devices(self) -> None:
        ...
        rtu_port = os.getenv("MODBUS_RTU_PORT") or "/dev/tty.usbserial-2130"
...
async def main() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("gateway_typed.log")],
    )
