...
try:
    from apps.edge.modbus.reader import KUB1063Reader
except Exception:
...
class TimeWindowManager:
    def __init__(
        ...
        serial_port="/dev/tty.usbserial-21230",
        **reader_kwargs,
    ):
        """
    ...
    with _manager_lock:
        if _time_window_manager is None:
            _time_window_manager = TimeWindowManager(
                serial_port=serial_port or "/dev/tty.usbserial-21230", **reader_kwargs
            )
...
if __name__ == "__main__":
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("logs/time_window_manager.log", encoding="utf-8"),
    ...
    parser.add_argument(
        "--port",
        dest="port",
        default="/dev/tty.usbserial-21230",
        help="Serial port path, e.g. /dev/tty.usbserial-21230",
    )
    parser.add_argument(
        "--window",
        dest="window",
