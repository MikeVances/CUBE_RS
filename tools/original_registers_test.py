#!/usr/bin/env python3
"""
Тест оригинальных адресов регистров КУБ-1063
"""
...
def read_single_register(register_address, host='localhost', port=5021):
    """
    Читает один регистр по Modbus TCP с локального gateway
    """
    try:
        # Создаем сокет
...
def main():
    """Основная функция для тестирования оригинальных адресов"""
    print("🔍 ТЕСТ ОРИГИНАЛЬНЫХ АДРЕСОВ РЕГИСТРОВ КУБ-1063")
    print("=" * 60)
    print("📍 Подключение к локальному gateway (localhost:5021)")
    print("=" * 60)
    
    # Оригинальные адреса регистров из документации
    test_registers = [
