...
class TestTypedModbusGatewayUnit:
    ...
    def gateway(self):
        ...
        return TypedModbusGateway(
            host="127.0.0.1", port=5023, db_path=":memory:", max_concurrent_requests=5
        )
    ...
    async def test_save_to_database(self, gateway, temp_db):
        ...
        response = ModbusResponse(
            request=request, success=True, data=[42, 100], response_time_ms=15.5
        )
    ...
    async def test_concurrent_requests(self, gateway):
        ...
        tasks = [gateway.modbus_client.execute_request(request) for _ in range(10)]
...
class TestTypedModbusGatewayPerformance:
    ...
    async def test_concurrent_performance(self, benchmark):
        ...
        async def execute_requests():
            ...
            tasks = [gateway.modbus_client.execute_request(request) for _ in range(20)]
