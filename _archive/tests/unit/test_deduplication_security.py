...
class TestDeduplicationSecurity:
    ...
    def test_no_arbitrary_code_execution_in_aliases(self):
        ...
        for alias_file in alias_files:
            ...
            assert (
                not dangerous_calls
            ), f"Dangerous calls in {alias_file}: {dangerous_calls}"
    ...
    def test_alias_modules_only_import_known_modules(self):
        ...
        for alias_file in alias_files:
            if (
                alias_file.name == "__init__.py"
                and len(alias_file.read_text().strip()) == 0
            ):
                ...
                if isinstance(node, (ast.Import, ast.ImportFrom)):
                    if isinstance(node, ast.ImportFrom) and node.module:
                        ...
                        if module_name not in safe_imports:
                            ...
                            pytest.fail(f"Unsafe import in {alias_file}: {node.module}")
...
class TestAliasSystemIntegrity:
        ...
        for alias_path, original_path in alias_mapping.items():
            try:
                ...
                original_obj = getattr(
                    importlib.import_module(original_module), original_class
                )
                ...
                try:
                    alias_obj = getattr(
                        importlib.import_module(alias_module), alias_class
                    )

                    # Should be the same object
                    assert (
                        original_obj is alias_obj
    ...
    def test_memory_usage_not_excessive(self):
        ...
        from core import security_manager, types  # noqa: F401
        from modbus import async_client, gateway_typed  # noqa: F401
        ...
        try:
            ...
            assert (
                memory_increase < baseline_memory * 0.5
