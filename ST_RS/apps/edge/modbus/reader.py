...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/reader.log", encoding="utf-8"),
...
REGISTER_MAP = {
    ...
    "factory_number": 0x0302,    # Заводской номер устройства
    "device_number": 0x0303,     # Номер устройства
    
    # Цифровые выходы (битовые поля)
    "digital_outputs_1": 0x0081,  # ГНВ базовой/туннельной вентиляции
    "digital_outputs_2": 0x0082,  # ГРВ, нагреватели, освещение, авария
    "digital_outputs_3": 0x00A2,  # Таймеры
    
    # Основные датчики
    "pressure": 0x0083,           # Отрицательное давление в 0.1 Па
    "humidity": 0x0084,           # Относительная влажность в 0.1%
    "co2": 0x0085,               # Концентрация CO2 в ppm
    "nh3": 0x0086,               # Концентрация NH3 в 0.1 ppm
    
    # Управляющие выходы ГРВ
    "grv_base": 0x0087,          # ГРВ базовой вентиляции в 0.1%
    "grv_tunnel": 0x0088,        # ГРВ туннельной вентиляции в 0.1%
    "damper": 0x0089,            # Демпфер в 0.1%
    
    # Воздухозаборники
    "air_intake_1": 0x008A,      # Воздухозаборник 1 в 0.1%
    "air_intake_2": 0x008B,      # Воздухозаборник 2 в 0.1%
    "air_intake_tunnel": 0x008C, # Туннельный воздухозаборник в 0.1%
    "air_intake_3": 0x0092,      # Воздухозаборник 3 в 0.1%
    "air_intake_4": 0x0093,      # Воздухозаборник 4 в 0.1%
    
    # Температурные датчики
    "temp_inside_1": 0x008D,     # Внутренняя температура 1 в 0.1°C
    "temp_inside_2": 0x008E,     # Внутренняя температура 2 в 0.1°C
    "temp_outside": 0x008F,      # Наружная температура в 0.1°C
    "temp_inside_3": 0x0090,     # Внутренняя температура 3 в 0.1°C
    "temp_inside_4": 0x0091,     # Внутренняя температура 4 в 0.1°C
    
    # Освещение
    "lighting_1": 0x0094,        # Управление освещением 1 в 0.1%
    "lighting_2": 0x0095,        # Управление освещением 2 в 0.1%
    "lighting_3": 0x0096,        # Управление освещением 3 в 0.1%
    "lighting_4": 0x0097,        # Управление освещением 4 в 0.1%
    
    # Таймеры
    "timer_1_output_1": 0x0098,  # Таймер 1, выход 1 в 0.1%
    "timer_1_output_2": 0x0099,  # Таймер 1, выход 2 в 0.1%
    "timer_1_output_3": 0x009A,  # Таймер 1, выход 3 в 0.1%
    "timer_1_output_4": 0x009B,  # Таймер 1, выход 4 в 0.1%
    "timer_2_output_1": 0x009C,  # Таймер 2, выход 1 в 0.1%
    "timer_2_output_2": 0x009D,  # Таймер 2, выход 2 в 0.1%
    "timer_2_output_3": 0x009E,  # Таймер 2, выход 3 в 0.1%
    "timer_2_output_4": 0x009F,  # Таймер 2, выход 4 в 0.1%
    
    # Аварии и предупреждения (битовые поля, 4 последовательных регистра по 16 бит)
    "active_alarms": 0x00C3,     # Активные аварии (группа 0x00C0-0x00C3)
    "registered_alarms": 0x00C7, # Зарегистрированные аварии (группа 0x00C4-0x00C7)
    "active_warnings": 0x00CB,   # Активные предупреждения (группа 0x00C8-0x00CB)
    "registered_warnings": 0x00CF, # Зарегистрированные предупреждения (группа 0x00CC-0x00CF)
    
    # Система вентиляции
    "ventilation_target": 0x00D0, # Целевой уровень вентиляции в 0.1%
    "ventilation_level": 0x00D1,  # Фактический уровень вентиляции в 0.1%
    "ventilation_scheme": 0x00D2, # Активная схема вентиляции (0-базовая, 1-туннельная)
    "day_counter": 0x00D3,        # Счетчик дней
    
    # Температурная система
    "temp_target": 0x00D4,        # Целевая температура вентиляции в 0.1°C
    "temp_inside": 0x00D5,        # Текущая температура вентиляции в 0.1°C
    "temp_vent_activation": 0x00D6, # Температура активации вентиляции в 0.1°C
}
...
class KUB1063Reader:
    ...
    def parse_value(self, raw_value: int, register_name: str) -> Any:
        ...
        # Обработка специальных значений согласно документации КУБ-1063
        # Для беззнаковых датчиков (pressure, humidity, co2, nh3, воздухозаборники, освещение, таймеры)
        if register_name in [
            "pressure", "humidity", "co2", "nh3",
            "grv_base", "grv_tunnel", "damper",
            "air_intake_1", "air_intake_2", "air_intake_tunnel", "air_intake_3", "air_intake_4",
            "lighting_1", "lighting_2", "lighting_3", "lighting_4",
            "timer_1_output_1", "timer_1_output_2", "timer_1_output_3", "timer_1_output_4",
            "timer_2_output_1", "timer_2_output_2", "timer_2_output_3", "timer_2_output_4"
        ]:
            if raw_value == 0xFFFF:
                return "pending"  # Измерение еще не выполнено
            elif raw_value == 0xFFFE:
                return "break"    # Обрыв датчика
            elif raw_value == 0xFFFD:
                return "error"    # Другие ошибки
            elif raw_value == 0xFFFC:
                return "disabled" # Датчик отключен в настройках
        
        # Для знаковых датчиков (температуры)
        elif register_name in [
            "temp_inside", "temp_target", "temp_vent_activation",
            "temp_inside_1", "temp_inside_2", "temp_outside", 
            "temp_inside_3", "temp_inside_4"
        ]:
            if raw_value == 0x7FFF:
                return "pending"  # Измерение еще не выполнено
            elif raw_value == 0x7FFE:
                return "break"    # Обрыв датчика
            elif raw_value == 0x7FFD:
                return "error"    # Другие ошибки
            elif raw_value == 0x7FFC:
                return "disabled" # Датчик отключен в настройках
        ...
        if register_name == "software_version":
        ...
        elif register_name == "factory_number":
            # Заводской номер в формате ММ.ГГ
            month = raw_value & 0xFF
            year = (raw_value >> 8) & 0xFF
            return f"{month:02d}.{year:02d}"

        elif register_name == "device_number":
            # Номер устройства (ННН)
            return raw_value
            ...
            "humidity", "pressure", "nh3",
            "grv_base", "grv_tunnel", "damper",
            "air_intake_1", "air_intake_2", "air_intake_tunnel", 
            "air_intake_3", "air_intake_4",
            "lighting_1", "lighting_2", "lighting_3", "lighting_4",
            "timer_1_output_1", "timer_1_output_2", "timer_1_output_3", "timer_1_output_4",
            "timer_2_output_1", "timer_2_output_2", "timer_2_output_3", "timer_2_output_4"
        ]:
            # Параметры в десятых долях процента (беззнаковые числа)
