...
try:
    from core.utils.paths import get_project_root, resolve_under_root
    PROJECT_ROOT = get_project_root(edge_root)
except Exception:
    PROJECT_ROOT = edge_root

from core.config_manager import get_config
from apps.edge.modbus.gateway_typed import TypedModbusGateway
...
async def main():
    ...
    # Resolve absolute DB path relative to EDGE root
    ...
    gateway = TypedModbusGateway(
        ...
        port=config.modbus_tcp.port,  # Use configured port
        db_path=db_path,
    ...
    try:
        ...
        logger.info(
            f"  • Modbus Write: POST http://0.0.0.0:{gateway.port}/modbus/write"
        )
