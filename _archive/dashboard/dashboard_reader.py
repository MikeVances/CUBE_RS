            ...
            try:
                ...
                if ar and getattr(ar, "enabled", False):
                    ...
                    if isinstance(val, int) and 0 <= bit <= 15:
                        ...
                        result["alarm_relay_label"] = getattr(
...
def get_historical_data(hours: int = 6) -> Optional[list]:
    ...
    try:
        with sqlite3.connect(DB_PATH, timeout=5.0) as conn:
            ...
            if not rows:
                logger.warning(
                    f"Нет исторических данных за последние {hours} часов"
                )
                return []
            ...
            for row in rows:
                ...
                record = {
                    "timestamp": timestamp,
                    "temp_inside": data.get("temp_inside")
                    if data.get("temp_inside") is not None
                    else None,
                    "temp_target": data.get("temp_target")
                    if data.get("temp_target") is not None
                    else None,
                    "humidity": data.get("humidity")
                    ...
                    "ventilation_level": data.get("ventilation_level")
                    if data.get("ventilation_level") is not None
                    else None,
                    "software_version": _parse_software_version(
            ...
            logger.info(
                f"Получено {len(historical_data)} исторических записей за {hours} часов"
            )
...
if __name__ == "__main__":
    ...
    if history:
        ...
        if len(history) > 0:
            ...
            last = history[-1]
            print(
                f"🕒 Период: {first['timestamp'].strftime('%H:%M:%S')} - {last['timestamp'].strftime('%H:%M:%S')}"
            )
            print(
                f"🌡️  Диапазон температур: {min(r['temp_inside'] for r in history if r['temp_inside']):.1f}°C - {max(r['temp_inside'] for r in history if r['temp_inside']):.1f}°C"
            )
    else:
