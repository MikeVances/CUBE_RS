...
class TelegramBotDB:
    ...
    def __init__(self, db_file: Optional[str] = None):
        ...
        if not Path(self.db_file).exists():
            logger.warning(
                f"⚠️ База данных {self.db_file} не найдена, создаем новую"
            )
            self._create_database()
        # Всегда обеспечиваем схему (на случай если файл был создан другими сервисами)
        try:
            self._ensure_schema()
        except Exception as e:
            logger.error(f"❌ Ошибка миграции схемы БД: {e}")

        logger.info(f"🗄️ Подключение к базе данных: {self.db_file}")
...
def find_user_by_username(self, username: str):
    ...
    try:
        ...
        with sqlite3.connect(self.db_file) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT * FROM telegram_users WHERE username = ?", (username,)
            )
            row = cursor.fetchone()
            return dict(row) if row else None
    except Exception as e:
