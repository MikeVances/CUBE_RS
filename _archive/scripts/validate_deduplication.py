...
def check_duplicate_content() -> dict[str, list[Path]]:
    ...
    for file_hash, files in hash_to_files.items():
        if len(files) > 1:
            ...
            for f in files:
                if str(f).startswith(
                    ("deploy_gateway/", "deploy_webapp/", "deploy_server/")
                ):
        ...
        if deploy_path.exists():
            for py_file in deploy_path.rglob("*.py"):
                if py_file.stat().st_size > 2000:  # Large files should be investigated
                    errors.append(
                        f"Large file in deploy directory: {py_file} ({py_file.stat().st_size} bytes)"
                    )
                ...
                for line in lines:
                    if not (
                        ...
                        or line.startswith(  # Simple class definitions for wrappers
                            "def "
                        )
                        or line.startswith("    ")  # Simple function definitions
                        or "=" in line  # Indented lines (class/function content)
                        and len(line) < 100
                        or line.startswith("__all__")  # Simple assignments
                        or line.startswith("[")  # __all__ lists
                        or line.startswith("]")
                        or line.startswith("'")  # List contents
                        or line.startswith('"')  # String literals
                    ):
                        significant_lines += 1
                ...
                threshold = (
                    20
                    if any(
                        word in py_file.name.lower()
                        for word in ["wrapper", "tool", "cli"]
                    )
                    else 10
                )
                if significant_lines > threshold:
                    errors.append(
                        f"Potentially duplicated code in: {py_file} ({significant_lines} non-import lines)"
                    )
...
def main() -> int:
        ...
        for file_hash, files in duplicates.items():
            if len(files) > 1:
                ...
                if len(significant_files) > 1:
                    errors.append(
                        f"Duplicate content found in: {[str(f) for f in significant_files]}"
                    )
    ...
    print(
        f"   📦 Backup files: {stats['backup_files']} ({stats['backup_size']:,} bytes)"
    )
    print(
        f"   🔗 Deploy files: {stats['deploy_files']} ({stats['deploy_size']:,} bytes)"
    )
    print(
        f"   💾 Space saved: {stats['space_saved']:,} bytes ({stats['percent_saved']}%)"
    )
    ...
    if stats["percent_saved"] < 80:
        print(
            f"⚠️  Warning: Space savings below 80% threshold ({stats['percent_saved']}%)"
        )
        return 1
