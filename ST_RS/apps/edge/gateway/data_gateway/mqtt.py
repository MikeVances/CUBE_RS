...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/mqtt.log", encoding="utf-8"),
...
def publish_loop():
    ...
    def on_connect(client, userdata, flags, rc):
        if rc == 0:
            logging.info(
                f"✅ Подключено к MQTT-брокеру {config.BROKER_HOST}:{config.BROKER_PORT}"
            )
        else:
    ...
    try:
        ...
        logging.info(
            f"🔗 Подключение к MQTT-брокеру {config.BROKER_HOST}:{config.BROKER_PORT}..."
        )
            ...
            try:
                ...
                if data:
                    ...
                    if result.rc == mqtt.MQTT_ERR_SUCCESS:
                        logging.info(
                            f"📤 Опубликовано в {config.MQTT_TOPIC}: {len(payload)} байт"
                        )
                    else:
