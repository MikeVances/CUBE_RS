...
ADDITIONAL_DUPLICATES = [
    ...
    (
        "deploy_webapp/tools/certificate_manager_cli.py",
        "tools/certificate_manager_cli.py",
    ),
    ...
    # Monitoring
    (
        "deploy_server/monitoring/network_security_monitor.py",
    ...
    (
        "deploy_webapp/web_app/production_device_registry.py",
        "web_app/production_device_registry.py",
    ),
    # Gateway auto registration
    (
        "deploy_gateway/gateway/auto_registration_client.py",
        "gateway/auto_registration_client.py",
    ),
]
...
def create_tool_wrapper(deploy_file: Path, original_file: Path) -> None:
    ...
    if not original_file.exists():
        print(
            f"   ⚠️  Original tool not found: {original_file}, keeping deploy version"
        )
        return
...
def create_simple_alias(deploy_file: Path, original_file: Path) -> None:
    ...
    if not original_file.exists():
        print(
            f"   ⚠️  Original file not found: {original_file}, keeping deploy version"
        )
        return
