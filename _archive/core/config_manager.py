...
class ConfigManager:
    ...
    def __init__(self, config_dir: str = "config"):
        ...
        # Modbus регистры: допускается как простая форма name->address,
        # так и расширенная: name->{address: str|int, display: bool}
        self.modbus_registers: dict[str, any] = {}
        ...
        if config_file.exists():
            try:
                ...
                if "modbus_registers" in config_data:
                    self.modbus_registers = config_data["modbus_registers"] or {}
    ...
    def get_modbus_register(self, name: str) -> Optional[str]:
        """Получение адреса Modbus регистра по имени (строкой)."""
        meta = self.get_modbus_registers_meta().get(name)
        if not meta:
            return None
        return meta.get("address")

    def get_all_modbus_registers(self) -> dict[str, str]:
        """Совместимый вид: name -> address (строкой)"""
        meta = self.get_modbus_registers_meta()
        return {k: v.get("address", "") for k, v in meta.items()}

    def get_modbus_registers_meta(self) -> dict[str, dict[str, any]]:
        """Нормализованная карта регистров: name -> {address: str, display: bool}

        Поддерживает 2 формата входного YAML:
        - простой: name: "0x00D5"
        - расширенный: name: {address: "0x00D5", display: true}
        """
        result: dict[str, dict[str, any]] = {}
        for name, val in (self.modbus_registers or {}).items():
            if isinstance(val, dict):
                addr = val.get("address")
                disp = bool(val.get("display", True))
            else:
                addr = val
                disp = True
            # Нормализуем адрес к строке
            if isinstance(addr, int):
                addr_str = hex(addr)
            else:
                addr_str = str(addr)
            result[name] = {"address": addr_str, "display": disp}
        return result
