...
Если нет точного текста, используется `label = "Авария <id>"`.
...
3. Документируйте новые агрегаты в этом файле или в docstring адаптера.
4. Соблюдайте безопасность: не логируйте секреты, не храните токены в коде.
