...
class TestModbusConnectionPool:
    ...
    async def connection_pool(self):
        ...
        pool = ModbusConnectionPool(
            max_connections=5, connection_timeout=2.0, idle_timeout=10.0
        )
        await pool.start()
        yield pool
        await pool.stop()
...
class TestAsyncModbusClient:
    ...
    @pytest.mark.asyncio
    async def test_disabled_device_request(
        self, modbus_client, tcp_device, read_request
    ):
    ...
    async def test_successful_read_request(
        self, mock_tcp_client_class, modbus_client, tcp_device, read_request
    ):
    ...
    @pytest.mark.asyncio
    @patch("modbus.async_client.AsyncModbusTcpClient")
    async def test_successful_write_request(
        self, mock_tcp_client_class, modbus_client, tcp_device, write_request
    ):
    ...
    @pytest.mark.asyncio
    @patch("modbus.async_client.AsyncModbusTcpClient")
    async def test_connection_failure_with_retry(
        self, mock_tcp_client_class, modbus_client, tcp_device, read_request
    ):
    ...
    @pytest.mark.asyncio
    @patch("modbus.async_client.AsyncModbusTcpClient")
    async def test_different_function_codes(
        self, mock_tcp_client_class, modbus_client, tcp_device
    ):
        ...
        mock_client.read_discrete_inputs = AsyncMock(
            return_value=mock_response_discrete
        )
...
class TestModbusClientPerformance:
    ...
    async def test_concurrent_requests(self, mock_tcp_client_class, modbus_client):
        ...
        for device in devices:
            ...
            assert (
                registered_device.total_requests >= 3
            )  # At least 3 requests per device
