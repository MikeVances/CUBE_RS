...
from typing import (
    ...
    Callable, Awaitable, AsyncGenerator, TypedDict, Literal, TYPE_CHECKING
)
...
class ModbusRequest(BaseModel):
    ...
    register_address: RegisterAddress = Field(..., ge=0, le=65535, description="Starting register address")
    register_count: int = Field(..., ge=1, le=125, description="Number of registers")
    write_values: Optional[List[RegisterValue]] = Field(None, description="Values to write (for write operations)")
