...
def format_sensor_data(data: dict[str, Any]) -> str:
    ...
    if sw_version or factory_number or device_number:
        text += f"{EMOJI['info']} Система: "
        if sw_version:
            text += f"v{sw_version} "
        if factory_number and device_number:
            text += f"№{factory_number}.{device_number:03d}"
        elif device_number:
            text += f"№{device_number}"
        text += "\n"
    text += "\n"
    ...
    if hum_status == "disabled" or not _CFG.sensors.get("humidity", True):
        ...
        if isinstance(humidity, str):
            # Обработка строковых статусов
            if humidity == "break":
                text += f"• Влажность: `обрыв датчика` {EMOJI['error']}\n"
            elif humidity == "error": 
                text += f"• Влажность: `ошибка измерения` {EMOJI['error']}\n"
            else:
                formatted_value = _format_sensor_value(humidity)
                text += f"• Влажность: {formatted_value}\n"
        else:
            # Численные значения
    ...
    if co2_status == "disabled" or not _CFG.sensors.get("co2", True):
        ...
        if isinstance(co2, str):
            # Обработка строковых статусов
            formatted_value = _format_sensor_value(co2)
            text += f"• CO₂: {formatted_value}\n"
        else:
            # Численные значения
...
def _format_sensor_value(value, unit="", is_temperature=False) -> str:
    """Форматирование значения датчика с обработкой статусов"""
    if value is None:
        return "нет данных"
    elif isinstance(value, str):
        # Строковые статусы датчиков
        status_map = {
...
def format_extended_sensor_data(data: dict[str, Any]) -> str:
    """Расширенное форматирование всех данных с датчиков КУБ-1063"""
    if not data:
        return f"{EMOJI['error']} **Нет данных от системы**"
    
    text = "📊 **РАСШИРЕННЫЕ ДАННЫЕ КУБ-1063**\n\n"
    
    # 1. Температурные датчики
    text += "**🌡️ ТЕМПЕРАТУРЫ:**\n"
    temp_fields = [
        ("temp_inside", "Основная внутренняя"),
        ("temp_target", "Целевая"),
        ("temp_outside", "Наружная"),
        ("temp_inside_1", "Внутренняя #1"),
        ("temp_inside_2", "Внутренняя #2"), 
        ("temp_inside_3", "Внутренняя #3"),
        ("temp_inside_4", "Внутренняя #4"),
        ("temp_vent_activation", "Активация вентиляции")
    ]
    
    for field, label in temp_fields:
        value = data.get(field)
        if value is not None:
            formatted_value = _format_sensor_value(value, "°C", is_temperature=True)
            text += f"• {label}: {formatted_value}\n"
    
    # 2. Воздухозаборники
    text += "\n**🌀 ВОЗДУХОЗАБОРНИКИ:**\n"
    air_fields = [
        ("air_intake_1", "Воздухозаборник #1"),
        ("air_intake_2", "Воздухозаборник #2"),
        ("air_intake_3", "Воздухозаборник #3"),
        ("air_intake_4", "Воздухозаборник #4"),
        ("air_intake_tunnel", "Туннельный")
    ]
    
    for field, label in air_fields:
        value = data.get(field)
        if value is not None:
            formatted_value = _format_sensor_value(value, "%")
            text += f"• {label}: {formatted_value}\n"
    ...
    for field, label in lighting_fields:
        value = data.get(field)
        if value is not None:
            formatted_value = _format_sensor_value(value, "%")
            text += f"• {label}: {formatted_value}\n"
    ...
    text += "\n**⏰ ТАЙМЕРЫ:**\n"
    timer_fields = [
        ("timer_1_output_1", "Таймер 1 - Выход 1"),
        ("timer_1_output_2", "Таймер 1 - Выход 2"),
        ("timer_1_output_3", "Таймер 1 - Выход 3"),
        ("timer_1_output_4", "Таймер 1 - Выход 4"),
        ("timer_2_output_1", "Таймер 2 - Выход 1"),
        ("timer_2_output_2", "Таймер 2 - Выход 2"),
        ("timer_2_output_3", "Таймер 2 - Выход 3"),
        ("timer_2_output_4", "Таймер 2 - Выход 4")
    ]
    
    for field, label in timer_fields:
        value = data.get(field)
        if value is not None:
            formatted_value = _format_sensor_value(value, "%")
            text += f"• {label}: {formatted_value}\n"
...
def build_main_menu(
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton("🔍 Подробно", callback_data="show_extended"),
