...
from core.types import (
    ...
    ModbusConnectionType, ModbusConnectionStatus, DeviceId, RegisterValue,
    ModbusError, CubeRSError
)
...
class ModbusConnectionPool:
    ...
    def __init__(
        ...
        logger.info(f"🔧 ModbusConnectionPool initialized: max={max_connections}, timeout={connection_timeout}s")
    ...
    async def _create_tcp_client(self, device: ModbusDevice) -> AsyncModbusTcpClient:
        ...
        try:
            ...
            if not client.connected:
                raise ConnectionException(f"Failed to connect to {conn.host}:{conn.port}")
    ...
    async def _create_serial_client(self, device: ModbusDevice) -> AsyncModbusSerialClient:
        ...
        try:
            await asyncio.wait_for(client.connect(), timeout=conn.timeout)
            if not client.connected:
                raise ConnectionException(f"Failed to connect to serial port {conn.host}")
        ...
        while True:
            try:
                ...
                for pool_key, stats in list(self._stats.items()):
                    if stats.last_used < cutoff_time and pool_key not in self._active_connections:
...
class AsyncModbusClient:
    ...
    async def execute_request(self, request: ModbusRequest) -> ModbusResponse:
        ...
        if not device:
            raise ModbusError(
                f"Device {request.device_id} not registered",
                request.device_id
            )

        if not device.enabled:
            raise ModbusError(
                f"Device {request.device_id} is disabled",
                request.device_id
            )
    ...
    async def _execute_read_request(
        ...
        async with self.pool.get_client(device) as client:
            ...
            try:
                ...
                if request.function_code in [ModbusFunctionCode.READ_COILS, ModbusFunctionCode.READ_DISCRETE_INPUTS]:
    ...
    async def _execute_write_request(
        ...
        if not request.write_values:
            raise ModbusError("Write values are required for write operations", request.device_id)
            ...
            try:
                if request.function_code == ModbusFunctionCode.WRITE_SINGLE_COIL:
                    if len(request.write_values) != 1:
                        raise ModbusError("Single coil write requires exactly one value", request.device_id)
                    ...
                    if len(request.write_values) != 1:
                        raise ModbusError("Single register write requires exactly one value", request.device_id)
