...
DUPLICATED_FILES = [
    ...
    ("deploy_webapp/web_app/tailscale_integration.py", "web_app/tailscale_integration.py"),
...
def verify_originals_exist(duplicated_files: List[Tuple[str, str]]) -> bool:
    ...
    for duplicate_path, original_path in duplicated_files:
        if Path(duplicate_path).exists() and not Path(original_path).exists():
            print(f"   ❌ Original missing: {original_path} (needed for {duplicate_path})")
            all_exist = False
...
"""
    ...
    alias_files = [
        ...
        ("deploy_webapp/web_app/api_gateway.py", "from web_app.api_gateway import *  # noqa: F403,F401"),
    ]
