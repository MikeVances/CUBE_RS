...
class UnifiedKUBSystem:
    ...
    def initialize_components(self):
        ...
        try:
            ...
            self.time_window_manager = get_time_window_manager(
                serial_port=rs485_config.get("port", "/dev/tty.usbserial-2130"),
            ...
            logger.info(
                "✅ Все чтение и запись будет происходить через TimeWindowManager"
            )
        ...
        try:
            ...
            if self.config.get("reader", {}).get("enabled", True):
                ...
                self.reader_thread = threading.Thread(
                    target=self._reader_loop, daemon=True, name="unified-reader"
                )
                ...
                self.stats_update_thread = threading.Thread(
                    target=self._stats_update_loop, daemon=True, name="unified-stats"
                )
                ...
                try:
                    register_thread_service(
                        name="unified_stats",
                        thread=self.stats_update_thread,
                        process_type=ProcessType.MONITORING,
    ...
    def _reader_loop(self):
        ...
        while self.is_running and not is_shutdown_requested():
            try:
                ...
                while (
                    data_result[0] is None
                    and error_result[0] is None
                ...
                if data_result[0]:
                    ...
                    if data.get("connection_status") == "connected":
                        ...
                        logger.info(
                            f"📊 Данные: T={temp}°C, H={humidity}%, CO2={co2}ppm"
                        )
                    else:
                ...
                if consecutive_errors >= max_retries:
                    ...
                    logger.error(
                        f"❌ Слишком много ошибок подряд, увеличиваем интервал до {error_interval}с"
                    )
                    if self.stop_event.wait(error_interval):
                        return
                    consecutive_errors = 0  # Сбрасываем после паузы
                else:
                    if self.stop_event.wait(read_interval):
                        return
        ...
        while self.is_running and not is_shutdown_requested():
            try:
                ...
                if self.writer:
                    ...
                    self.system_stats["writer_commands"] = writer_stats.get(
                        "commands_total", 0
                    )

                    # Проверяем последнюю успешную запись
                    if writer_stats.get("last_command_time"):
                        self.system_stats["last_successful_write"] = writer_stats[
                            "last_command_time"
                        ]

                if self.stop_event.wait(60):  # Обновляем статистику каждую минуту
                    return
    ...
    def stop(self):
        ...
        self.is_running = False
        self.stop_event.set()  # Прерываем все ожидания
...
def main():
    ...
    try:
        ...
        while True:
            ...
            stats = system.get_system_statistics()
            print(
                f"📊 Uptime: {stats['system']['uptime_seconds']}s, "
